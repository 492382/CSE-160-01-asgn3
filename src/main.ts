...
function render(milis: number) {
  ...
  let matrix1_2 =
      matrix_multiply(
	rotor_to_matrix(make_rotation_rotor((milis / 1000), [Math.sqrt(2), 0, Math.sqrt(2)])),
	make_scale_matrix(3, 3, 3)
    );
  ...
  let matrix2 = matrix_multiply(
    make_translation_matrix(0, -3, 0),
    make_scale_matrix(50, 0.5, 50));
